...
function allCC(e) {
  console.log(
    "controller number = " + e.controller.number + ", value = " + e.data[2],
  );
  ...
  switch (e.controller.number) {
    ...
    case CCSLIDER2:
      ...
      break;
    case CCSLIDER3:
      flowMix = map(e.data[2], 0, 127, 0, 1);
      break;
    case CCSLIDER4:
...
function draw() {
  ...
  for (var i = 0; i <= 200; i++) {
    ...
    for (var j = 0; j <= 300; j += 60) {
      ...
      timeOffset += 0.01 * flowMix;

      let radP = i * scaleFactor;

      // Base pattern position
      let px = radP * cos(j);
      let py = radP * sin(j);
      let pz = cos(frameCount * 10 + 4 * i) * 50;

      // Flow field offset
      let angle =
        noise(px * noiseScale, py * noiseScale, timeOffset) * TWO_PI * 2;

      let fx = px + cos(angle) * 80;
      let fy = py + sin(angle) * 80;
      let fz = pz + sin(angle + timeOffset) * 60;

      // Blend between pattern and flow
      let a = lerp(px, fx, flowMix);
      let b = lerp(py, fy, flowMix);
      let c = lerp(pz, fz, flowMix);
